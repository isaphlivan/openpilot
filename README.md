...
### Opel Corsa F (2020+) Support
**[TR]** Flowpilot için Opel Corsa F (2020 ve üzeri) modelleri (PSA CMP platformu) donanım desteği eklenmiştir. Özel CAN mesajları (LDW, LKAS, HUD, ACC) tanımlanmış, `test_opel_sim.py` ile birim / simülasyon testleri başarıyla gerçekleştirilmiş ve Tork güvenlik kısıtlamaları (Max 3.00 Nm) ayarlanmıştır. Çalıştırmadan önce sahadaki aracınızın gerçek Firmware hex kodlarını `values.py` içerisine girmeyi unutmayın.

**[EN]** Added hardware support for Opel Corsa F (2020 and later) models based on the PSA CMP platform. Custom CAN messages (LDW, LKAS, HUD, ACC) have been defined, unit/simulation test `test_opel_sim.py` has run successfully, and Torque security limits (Max 3.00 Nm) are configured safe-by-design. Remember to update the placeholder Firmware hex codes in `values.py` with your real car's OBD readings before road testing.
